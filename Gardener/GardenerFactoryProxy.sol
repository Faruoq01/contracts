...
contract UpgradeableProxy {
    ...
    address[] public gardenerImplementations;
    uint256 public currentIndex;

    // gardener reference
    address public gardenerContract;
    bool public isGardenerAssigned;

    constructor(address[] memory _admins, address _implementation, address[] memory _gardenerImplementations) {
        ...
        gardenerImplementations = _gardenerImplementations;
    }
    ...
    function getNextGardenImplementationAddress() external returns (uint256) {
        if (gardenerImplementations.length == 0) {
            revert("No addresses available");
        }

        // Update the index to the next one in a round-robin fashion
        currentIndex = (currentIndex + 1) % gardenerImplementations.length;
        return currentIndex;
    }
